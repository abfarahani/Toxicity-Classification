...
with open("trainWordStatsFilterDel.csv",mode='r') as input:
	allLines = input.read()
	input.close()
...
allWordList.sort()
with open('wordIdxListFilterDel.txt',mode='w') as output:
...
for i in range(0,10):
	...
	with open("testFilterDel"+str(i)+".libsvm",mode="w") as test:
		test.write(testLines)
		test.close()
	with open("trainFilterDel"+str(i)+".libsvm",mode="w") as train:
		train.write(trainLines)
		train.close()

# Finally, write out the whole libsvm
with open("trainFilterDelAll.libsvm",mode="w") as train:
	train.write("\n".join(outputLines))
	train.close()
...
with open("testFilterDelAll.libsvm",mode="w") as train:
	train.write("\n".join(outputLines))
	train.close()
