...
with open("trainWordStatsFilterSingQuoteTogether.csv",mode='r') as input:
	allLines = input.read()
	input.close()
...
allWordList.sort()
with open('wordIdxListFilterSingQuoteTogether.txt',mode='w') as output:
...
for i in range(0,10):
	...
	with open("testFilterSingQuote"+str(i)+".libsvm",mode="w") as test:
		test.write(testLines)
		test.close()
	with open("trainFilterSingQuote"+str(i)+".libsvm",mode="w") as train:
		train.write(trainLines)
		train.close()

# Write out the whole libsvm
with open("trainFilterSingQuoteAll.libsvm",mode="w") as train:
	train.write("\n".join(outputLines))
	train.close()
...
with open("testFilterSingQuoteAll.libsvm",mode="w") as train:
	train.write("\n".join(outputLines))
	train.close()
