...
with open("trainWordStatsFilterDelSingQuote.csv",mode='r') as input:
	allLines = input.read()
	input.close()
...
allWordList.sort()
with open('wordIdxListFilterDelSingQuote.txt',mode='w') as output:
...
for i in range(0,10):
	...
	with open("testFilterDelSingQuote"+str(i)+".libsvm",mode="w") as test:
		test.write(testLines)
		test.close()
	with open("trainFilterDelSingQuote"+str(i)+".libsvm",mode="w") as train:
		train.write(trainLines)
		train.close()

# Finally, write out the whole libsvm
with open("trainFilterDelSingQuoteAll.libsvm",mode="w") as train:
	train.write("\n".join(outputLines))
	train.close()
...
with open("testFilterDelSingQuoteAll.libsvm",mode="w") as train:
	train.write("\n".join(outputLines))
	train.close()
